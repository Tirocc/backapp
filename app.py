...
with col2:
    st.header("❄️ Froster")
    st.subheader("Bedarf (Bleche)")
    muehlen_bl = st.number_input("Mühlenrädle (Bedarf)", min_value=0, step=1)
    milch_bl = st.number_input("Milchbrötchen (Bedarf)", min_value=0, step=1)
    schnitt_bl = st.number_input("Schnittbrötchen (Bedarf)", min_value=0, step=1)
    
    st.markdown("---")
    st.subheader("📦 Bestand abziehen (Bleche)")
    bestand_muehlen = st.number_input("Bestand Mühlenrädle", min_value=0, step=1)
    bestand_milch = st.number_input("Bestand Milchbrötchen", min_value=0, step=1)
    bestand_schnitt = st.number_input("Bestand Schnittbrötchen", min_value=0, step=1)
...
with col3:
    ...
    schnitt_so_bl = st.number_input("+ Schnittbrötchen So. (Bedarf Bleche)", min_value=0, step=1)
...
if st.button("🚀 Alles Berechnen", use_container_width=True, type="primary"):
    ...
    # 2. Froster-Bestand abziehen (Netto-Bleche berechnen, Minimum ist 0)
    netto_muehlen_bl = max(0, muehlen_bl - bestand_muehlen)
    netto_milch_bl = max(0, milch_bl - bestand_milch)
    netto_schnitt_bl = max(0, (schnitt_bl + schnitt_so_bl) - bestand_schnitt)

    # 3. In Stück umrechnen für die Teig- & Wagenberechnung
    summe_muehlen = netto_muehlen_bl * produkte["Mühlenrädle"]["stueck_pro_blech"]
    summe_milch = netto_milch_bl * produkte["Milchbrötchen"]["stueck_pro_blech"]
    summe_schnitt = netto_schnitt_bl * produkte["Schnittbrötchen"]["stueck_pro_blech"]
    ...
    with out_col2:
        ...
        teig_auftraege = [
            ...
            ("Mühlenrädle", summe_muehlen, "Mühlenrädle (Netto)"),
            ("Milchbrötchen", summe_milch, "Milchbrötchen (Netto)"),
            ("Schnittbrötchen", summe_schnitt, "Schnittbrötchen (Netto)")
        ]
